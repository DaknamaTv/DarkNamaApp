package com.pira.darknama.data.repository

import com.pira.darknama.data.model.FilterType
import com.pira.darknama.data.model.Poster
    ...
    private fun parsePoster(posterObj: JSONObject): Poster {
        return Poster(
            ...
            genres = try {
                ...
                val genres = mutableListOf<com.pira.darknama.data.model.Genre>()
                for (j in 0 until genresArray.length()) {
                    val genreObj = genresArray.getJSONObject(j)
                    genres.add(
                        com.pira.darknama.data.model.Genre(
            ...
            sources = try {
                ...
                val sources = mutableListOf<com.pira.darknama.data.model.Source>()
                for (j in 0 until sourcesArray.length()) {
                    val sourceObj = sourcesArray.getJSONObject(j)
                    sources.add(
                        com.pira.darknama.data.model.Source(
            ...
            country = try {
                ...
                val countries = mutableListOf<com.pira.darknama.data.model.Country>()
                for (j in 0 until countriesArray.length()) {
                    val countryObj = countriesArray.getJSONObject(j)
                    countries.add(
                        com.pira.darknama.data.model.Country(
